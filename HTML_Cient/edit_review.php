...
if ($review_id > 0) {
    $stmt = $conn->prepare("SELECT r.*, br.bookId, b.title 
                           FROM reviews r
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($message)) {
        ...
        // Update review
        $stmt = $conn->prepare("UPDATE reviews SET message = ?, updatedAt = NOW() WHERE id = ?");
...
<head>
    ...
    <title>Edit Review - <?= htmlspecialchars($review['title']) ?></title>
...
<body>
    ...
    <button class="floating-back" onclick="window.history.back()">
    ...
    <div class="container">
        <div class="review-form">
            <div class="form-header">
                ...
                <p>Update your thoughts about "<?= htmlspecialchars($review['title']) ?>"</p>
            </div>
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <textarea id="message" name="message" placeholder="Write your review here..." required><?= 
                        isset($_POST['message']) ? htmlspecialchars($_POST['message']) : htmlspecialchars($review['message']) 
                    ?></textarea>
                    <div class="char-count"><span id="char-counter"><?= strlen($review['message']) ?></span> characters</div>
                </div>
    ...
    <script>
        ...
        document.querySelector('.floating-back').addEventListener('click', function() {
            if (window.history.length > 1) {
                ...
                window.location.href = 'review_book.php?id=<?= $review['bookId'] ?>';
            }
        });
    </script>
</body>
