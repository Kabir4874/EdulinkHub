    ...
    <main class="main-content">
        ...
        <div class="content">
            <div class="page-header">
                <h1 class="page-title"><i class="fa-solid fa-books"></i> Book List</h1>
            ...
            <div class="toolbar">
                <input id="search" class="input" type="search" placeholder="Search by title, author, or keywords…" />
                ...
                <select id="paidFilter" class="select">
            ...
            <div class="card">
                <div class="table-wrap">
                    <table id="bookTable">
                        ...
                        <tbody id="bookTbody">
                            <?php /*
                PHP LOOP PLACEHOLDER — render real rows later from the `books` table:
                  ...
                  <tr>
                    ...
                    <td>
                      <?php if (!empty($b['image'])): ?>
                        <img class="cover" src="<?= htmlspecialchars($b['image']) ?>" alt="<?= htmlspecialchars($b['title']) ?>">
                      <?php endif; ?>
                    </td>
                    ...
                    <td>
                      <?php $isPaid = (int)$b['isPaid'] === 1; ?>
                      <span class="badge <?= $isPaid ? 'paid' : 'free' ?>"><?= $isPaid ? 'Paid' : 'Free' ?></span>
                    </td>
                    <td><?= htmlspecialchars($b['uploadDate']) ?></td>
                    <td>
                      <?php if (!empty($b['pdf'])): ?>
              ...
              */ ?>
                            ...
                            <tr class="empty-row">
                                <td colspan="12" style="text-align:center; color:#64748b; padding:24px;">
                                    No books found yet. Add a new book to see rows here.
                                </td>
                            </tr>
              ...
              <tr>
                <td>1</td>
                <td><img class="cover" src="https://via.placeholder.com/90x120" alt="Sample Book"></td>
                ...
                <td>2025-08-01</td>
                <td><a class="link" href="#" target="_blank" rel="noopener">Open PDF</a></td>
    ...
    <script>
        ...
        (function() {
            ...
            function applyFilters() {
                ...
                const cat = categoryFilter.value; // "", "Admission", "Job Exam", "Skill-Based"
                const paidSel = paidFilter.value; // "", "paid", "free"

                filtered = rows.filter(tr => {
                    const tds = tr.querySelectorAll('td');
                    if (!tds.length) return false;

                    // Searchable columns: title (2), author (3)
                    const hay = [
                        tds[2]?.textContent || '',
                        tds[3]?.textContent || ''
                    ].join(' ').toLowerCase();

                    const catText = (tds[4]?.textContent || '').trim();
                    const paidText = (tds[6]?.textContent || '').trim().toLowerCase(); // "paid" / "free"

                    const matchQ = q === '' || hay.includes(q);
                    const matchC = cat === '' || catText === cat;
                    const matchP = paidSel === '' || paidText === paidSel;
